/**
 * Meera — agent core. Pure logic: no HTTP layer, no process-local state.
 *
 * Shared by the local dev server (app/server.js) and the serverless functions in api/.
 * Speech is addressed by an encoded URL rather than a server-side map, so a synthesis
 * request can be served by a different instance than the turn that produced it — the
 * requirement that makes this deployable on serverless at all.
 */
...
export function assertKey() {
  if (!KEY) throw new Error('SARVAM_API_KEY is not set on this deployment');
}

/** Encode the whole synthesis request into the URL — no shared server state. */
export function speechLink(text, lang, speaker, tone = 'warm') {
  const p = Buffer.from(JSON.stringify({ t: text, l: lang, s: speaker, o: tone }), 'utf8')
    .toString('base64url');
  return `/api/speak?p=${p}`;
}
export function decodeSpeech(p) {
  const o = JSON.parse(Buffer.from(String(p), 'base64url').toString('utf8'));
  return { text: o.t, lang: o.l, speaker: o.s, tone: o.o };
}
...
export async function speechStream({ text, lang, speaker, tone }) {
...
Address her as "ma'am" or "ji" — never "sir". In English use "Ms." with the surname.`;
  ...
  const speechUrl = spoken
    ? speechLink(spoken, lang, body.speaker || 'shreya', slots?.tone)
    : null;
  t.total = Date.now() - t0;

  return {
    transcript, reply: spoken, speechUrl, slots, timings: t,
...
export async function handleGreeting(body) {
  ...
  return {
    reply: text,
    speechUrl: speechLink(text, 'en-IN', body.speaker || 'shreya', 'warm'),
    lang: 'en-IN',
  };
}
